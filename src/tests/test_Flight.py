...
class TestFlightClass(unittest.TestCase):
    ...
    def test_init(self):
        ...
        response = requests.get("https://api.skypicker.com/flights?flyFrom=MEL&to=SYD&curr=AUD&dateFrom=12/12/2020&dateTo=15/12/2020&partner=picky&v=3")
        data = json.loads(response.text)
        test_response = data["data"][0]
        test_flight = Flight(test_response)
        # Test attributes are being initialized
